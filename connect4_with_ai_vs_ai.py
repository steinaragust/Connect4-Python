...
def evaluate_window(window, piece):
    score = 0
    opp_piece = MC_AI_PIECE
    if piece == MC_AI_PIECE:
        opp_piece = AI_PIECE
...
def is_terminal_node(board):
    return (
        winning_move(board, MC_AI_PIECE)
...
def minimax(board, depth, alpha, beta, maximizingPlayer):
    ...
    if depth == 0 or is_terminal:
        if is_terminal:
            ...
            elif winning_move(board, MC_AI_PIECE):
    ...
    if maximizingPlayer:
        ...
        for col in valid_locations:
            row = get_next_open_row(board, col)
            b_copy = board.copy()
            drop_piece(b_copy, row, col, MC_AI_PIECE)
    ...
    while True:
        ...
        if turn == MC_AI:
            ...
            if is_valid_location(board, col):
                ...
                row = get_next_open_row(board, col)
                drop_piece(board, row, col, MC_AI_PIECE)
